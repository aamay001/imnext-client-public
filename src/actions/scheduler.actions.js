...
export const HUMAN_VALIDATION_REQUEST_FAILED = 'HUMAN_VALIDATION_REQUEST_FAILED';
...
export const getValidationCode = (formData) => dispatch => {
  ...
  const request = new Request(API.URL + API.HUMAN_VALIDATION, init);
  fetch(request)
  .then( res => {
  ...
  .catch(error => {
    dispatch(humanValidationRequestFailed({
...
export const getAvailableTimeSlots = (date, provider) => dispatch => {
  ...
  const request = new Request(
    `${API.URL + API.AVAILABLE_TIMES(provider,date)}`,
    init
  );
  fetch(request)
  .then( res => {
...
export const scheduleAppointment = formData => dispatch => {
  ...
  let request = new Request(API.URL + API.VALIDATE_APPOINTMENT, init);
  fetch(request)
  .then( res => {
    if (!res.ok) {
      return res.json().then(data =>
  ...
  .then(data => {
    ...
    request = new Request(API.URL + API.APPOINTMENT, init);
